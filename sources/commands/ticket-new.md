...
# Check if {{TICKET_PREFIX}}-$NEXT already exists
if ls {{TASKS_DIR}}/{{TICKET_PREFIX}}-$(printf "%03d" $NEXT)-* 2>/dev/null; then
  echo "{{TICKET_PREFIX}}-$NEXT exists! Finding next available..."
