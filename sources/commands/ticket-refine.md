...
### Completed Work
- JWT authentication backend ({{TICKET_PREFIX}}-013, merged)
- Login UI component ({{TICKET_PREFIX}}-014, on branch feature/login-ui)
...
2. **Create subtask in {{TICKET_PREFIX}}-XXX/ folder:**
...
```markdown
   ### Subtasks
   - [x] {{TICKET_PREFIX}}-013: JWT setup (DONE)
   - [x] {{TICKET_PREFIX}}-014: Login UI (DONE)
   - [ ] {{TICKET_PREFIX}}-015: Password reset flow (TODO)
   - [ ] {{TICKET_PREFIX}}-016: Session timeout (TODO)
```
...
- [ ] {{TICKET_PREFIX}}-015: Password reset flow
- [ ] {{TICKET_PREFIX}}-016: Session timeout handling

### Next Steps
Recommend starting with {{TICKET_PREFIX}}-015 (password reset) as it's independent.
```
