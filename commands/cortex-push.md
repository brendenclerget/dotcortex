...
2. Copy the asset from the team layer into the org base at the right profile path (`base/pm/skills/<name>/`, `base/pm/commands/<name>.md`, or the knowledge equivalent your org base defines). **De-brand on the way in**: replace project/team-specific values with the render tokens (`{{TICKET_PREFIX}}`, `{{TASKS_DIR}}`, …) and run `scripts/check-debrand.sh` on the changed paths.
