...
argument-hint: <PREFIX-XXX> <STATUS> [OWNER]
---
...
`/ticket-status <PREFIX-XXX> <STATUS> [OWNER]`
...
- **Parent directory:** `TASKS_DIR/PREFIX-XXX/PREFIX-XXX-name.md` (and child files inside)
...
Read `TASKS_DIR/BACKLOG.md` and move the ticket entry to the section that matches its new status, per the backlog-cleanup skill's categorization rules. If the ticket isn't tracked in the backlog (e.g., a child ticket), skip silently.
