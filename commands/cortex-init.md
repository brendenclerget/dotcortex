...
4. **Migrate bootstrap commands into the layer** so the resolved view can own `.dotcortex/commands`: `mkdir -p .dotcortex/layers/org/commands && mv .dotcortex/commands/*.md .dotcortex/layers/org/commands/` (then remove the now-empty `.dotcortex/commands` dir).
