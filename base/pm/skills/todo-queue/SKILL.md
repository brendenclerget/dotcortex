...
- **Sync**: Re-read every ticket and repair stale title/status/gate fields without changing order.
  Report missing, archived, duplicated, or newly blocked IDs rather than guessing. For rows
  carrying a Linear issue ID, apply the Linear block (see `pm-agent` skill) — action: refresh the
  row's status, assignment, and priority from the Linear issue. **`/todo sync` NEVER reorders the
  queue.** Linear augments row fields; queue order is owned by this file and wins every ordering
  conflict, including against Linear priority.
...
- A row may carry a Linear issue ID alongside its {{TICKET_PREFIX}} ID (top-level tickets only —
  letter-suffix children never get one). The Linear ID is a reference field, not a second identity:
  the row is still keyed by its {{TICKET_PREFIX}} ID, and rows without a Linear ID are normal.
...
- Commit/push task-repo queue changes immediately as one scoped transaction per operation, staging
  only the files this operation owns (never `git add -A`). See `pm-agent` → **Team Sync**.
